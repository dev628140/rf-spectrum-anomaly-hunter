...
export function ModelComparison() {
  ...
  return (
    <Card className="p-8 border-cyan-500/10 bg-[#07111f] shadow-[0_0_50px_rgba(0,255,255,0.02)] h-full flex flex-col justify-between rounded-[2rem]">
      <div className="flex flex-col h-full justify-between">
        ...
        <CardHeader className="flex flex-row items-center justify-between mb-6 pb-0 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-4xl font-black text-white">Consensus Threat Verification</CardTitle>
              <div className="text-slate-400 text-sm mt-1">Multi-model consensus monitoring deck</div>
            </div>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 rounded-full border border-cyan-500/20 bg-cyan-500/10 text-cyan-300 font-bold text-sm tracking-widest font-mono uppercase">
            <span className="h-2 w-2 rounded-full bg-cyan-400 animate-ping" />
            Active
          </div>
        </CardHeader>

        {/* SCROLLABLE CONTENT BLOCK (max-h-[460px]) */}
        <CardContent className="space-y-6 mt-4 max-h-[460px] overflow-y-auto pr-2 custom-scrollbar flex-1">
          <div className="grid grid-cols-1 gap-6">

            {/* CARD 1: AUTOENCODER */}
            <div className={`border rounded-3xl p-6 transition-all duration-300 bg-black/25 ${
              isAEActive 
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="space-y-3.5 flex-1 w-full">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-2xl font-black text-white leading-none">Neural Reconstruction Engine</span>
                    <span className="text-xs font-black text-slate-300 tracking-wider">Deep Learning</span>
                    {isAEActive && (
                      <span className="px-2.5 py-0.5 rounded bg-cyan-500 text-black text-[9px] font-black tracking-widest font-mono uppercase shadow-[0_0_10px_rgba(6,182,212,0.3)]">
                        ACTIVE
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-end gap-8 mt-2">
                    <div>
                      <span className="text-xs text-slate-300 font-black tracking-widest block uppercase mb-1">Baseline Match</span>
                      <span className={`text-3xl font-black font-mono leading-none ${isAEAnomaly ? "text-red-400 animate-pulse" : "text-cyan-400"}`}>
                        {aeMatchPercent}%
                      </span>
                    </div>
                    <div>
                      <span className="text-xs text-slate-300 font-black tracking-widest block uppercase mb-1">Telemetry State</span>
                      {isAEAnomaly ? (
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-red-500/10 border border-red-500/25 text-red-400 text-xs font-black tracking-widest font-mono uppercase">
                          <ShieldAlert className="h-3.5 w-3.5 animate-pulse" />
                          Anomaly
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-cyan-500/10 border border-cyan-500/25 text-cyan-300 text-xs font-black tracking-widest font-mono uppercase">
                  ...
                  <div className="text-xs font-mono text-slate-400 font-bold pt-1">
                ...
                <div className="w-[130px] h-[60px] border border-cyan-500/15 bg-black/40 rounded-xl relative overflow-hidden shrink-0 flex items-center justify-center p-2">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="space-y-3.5 flex-1 w-full">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-2xl font-black text-white leading-none">Supervised Threat Classifier</span>
                    <span className="text-xs font-black text-slate-300 tracking-wider">Decision Trees</span>
                    {isRFActive && (
                      <span className="px-2.5 py-0.5 rounded bg-purple-500 text-white text-[9px] font-black tracking-widest font-mono uppercase shadow-[0_0_10px_rgba(168,85,247,0.3)]">
                        ACTIVE
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-end gap-8 mt-2">
                    <div>
                      <span className="text-xs text-slate-300 font-black tracking-widest block uppercase mb-1">Threat Match</span>
                      <span className={`text-3xl font-black font-mono leading-none ${isRFAnomaly ? "text-red-400 animate-pulse" : "text-purple-400"}`}>
                        {rfMatchPercent}%
                      </span>
                    </div>
                    <div>
                      <span className="text-xs text-slate-300 font-black tracking-widest block uppercase mb-1">Telemetry State</span>
                      {isRFAnomaly ? (
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-red-500/10 border border-red-500/25 text-red-400 text-xs font-black tracking-widest font-mono uppercase animate-pulse">
                          <ShieldAlert className="h-3.5 w-3.5" />
                          {modelScores.random_forest?.threat_type?.replace("_", " ") || "Threat Confirmed"}
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-purple-500/10 border border-purple-500/25 text-purple-300 text-xs font-black tracking-widest font-mono uppercase">
                          <CheckCircle2 className="h-3.5 w-3.5" />
                          Normal
                        </div>
                      )}
                ...
                <div className="w-[130px] h-[60px] border border-purple-500/15 bg-black/40 rounded-xl relative shrink-0 flex items-center justify-center p-2">
                  <div className="grid grid-cols-5 gap-2.5 w-full h-full max-w-[110px] max-h-[40px]">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="space-y-3.5 flex-1 w-full">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-2xl font-black text-white leading-none">Distance-Based Outlier Core</span>
                    <span className="text-xs font-black text-slate-300 tracking-wider">KNN Space Cluster</span>
                    {isKNNActive && (
                      <span className="px-2.5 py-0.5 rounded bg-teal-500 text-black text-[9px] font-black tracking-widest font-mono uppercase shadow-[0_0_10px_rgba(20,184,166,0.3)]">
                        ACTIVE
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-end gap-8 mt-2">
                    <div>
                      <span className="text-xs text-slate-300 font-black tracking-widest block uppercase mb-1">Cluster Proximity</span>
                      <span className={`text-3xl font-black font-mono leading-none ${isKNNAnomaly ? "text-red-400 animate-pulse" : "text-teal-400"}`}>
                        {knnProximityPercent}%
                      </span>
                    </div>
                    <div>
                      ...
                      {isKNNAnomaly ? (
                        ...
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-teal-500/10 border border-teal-500/25 text-teal-300 text-xs font-black tracking-widest font-mono uppercase">
                          <CheckCircle2 className="h-3.5 w-3.5" />
                          Cluster In
                        </div>
                      )}
                ...
                <div className="w-[130px] h-[60px] border border-teal-500/15 bg-black/40 rounded-xl relative shrink-0 flex items-center justify-center p-2">
                  <div className="absolute inset-0 bg-[linear-gradient(rgba(20,184,166,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(20,184,166,0.02)_1px,transparent_1px)] bg-[size:10px_10px] pointer-events-none" />
                  <svg className="w-full h-full" viewBox="0 0 115 50">
        ...
        {/* FOOTER */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-white/5 text-xs font-bold font-mono text-slate-500 shrink-0">
